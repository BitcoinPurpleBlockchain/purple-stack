...
_NETWORK_INFO  = {'version': 110_100, 'subversion': '/BitcoinPurple:1.1.1/', 'connections': 8}
_MINING_INFO   = {'networkhashps': 9_876_543_210.0, 'difficulty': 1_234_567.89}
_PEER_INFO     = [{'addr': '1.2.3.4:13496', 'inbound': False, 'subver': '/BitcoinPurple:1.1.1/',
...
class TestApiEndpoints:
    ...
    @patch('app.bitcoinpurple_rpc_call', side_effect=_rpc)
    ...
    @patch('app.bitcoinpurple_rpc_call', side_effect=_rpc)
    def test_bitcoinpurple_info(self, _mock, client):
        r = self._get(client, '/api/bitcoinpurple/info')
        assert r.status_code == 200
        data = r.get_json()
    ...
    def test_block_height(self, _mock, client):
        r = self._get(client, '/api/bitcoinpurple/block-height')
        assert r.status_code == 200
        data = r.get_json()
    ...
    def test_network_hashrate(self, _mock, client):
        r = self._get(client, '/api/bitcoinpurple/network-hashrate')
        assert r.status_code == 200
        data = r.get_json()
    ...
    def test_difficulty(self, _mock, client):
        r = self._get(client, '/api/bitcoinpurple/difficulty')
        assert r.status_code == 200
        data = r.get_json()
    ...
    def test_coinbase_subsidy(self, _mock, client):
        r = self._get(client, '/api/bitcoinpurple/coinbase-subsidy')
        assert r.status_code == 200
        data = r.get_json()
    ...
    def test_peers(self, _mock, client):
        r = self._get(client, '/api/bitcoinpurple/peers')
        assert r.status_code == 200
        data = r.get_json()
    ...
    def test_recent_blocks(self, _mock, client):
        r = self._get(client, '/api/bitcoinpurple/blocks/recent')
        assert r.status_code == 200
        data = r.get_json()
...
class TestCacheHeaders:
    ...
    def test_no_store_header_present(self, _mock, client):
        r = client.get('/api/bitcoinpurple/block-height',
    ...
    def test_pragma_no_cache(self, _mock, client):
        r = client.get('/api/bitcoinpurple/info',
                       environ_base={'REMOTE_ADDR': TRUSTED_IP})
