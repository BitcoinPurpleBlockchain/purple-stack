...
def get_electrumx_service_ports():
    ...
    try:
        ...
        result = subprocess.run(
            ['docker', 'exec', 'bitcoinpurple-electrumx', 'sh', '-c', 'printf "%s" "${SERVICES:-}"'],
            capture_output=True,
            text=True,
            timeout=2
        )
...
def get_electrumx_stats(include_addnode_probes=False):
    ...
    try:
        ...
        try:
            result = subprocess.run(
                ['docker', 'exec', 'bitcoinpurple-electrumx', 'sh', '-c',
        ...
        try:
            ...
            result = subprocess.run(
                ['docker', 'inspect', 'bitcoinpurple-electrumx', '--format', '{{.State.StartedAt}}'],
                capture_output=True,
                text=True,
                timeout=2
            )
        ...
        try:
            result = subprocess.run(
                ['docker', 'exec', 'bitcoinpurple-electrumx', 'du', '-sb', '/data'],
                capture_output=True,
                text=True,
                timeout=5
            )
