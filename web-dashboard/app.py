#!/usr/bin/env python3
"""
Web Dashboard API for BitcoinPurple Node and ElectrumX Server Statistics
"""
...
def unauthorized_response():
    ...
    response.headers['WWW-Authenticate'] = 'Basic realm="BitcoinPurple Dashboard"'
    return response
...
# Configuration
BITCOINPURPLE_RPC_HOST = os.getenv('BITCOINPURPLE_RPC_HOST', 'bitcoinpurpled')
BITCOINPURPLE_RPC_PORT = int(os.getenv('BITCOINPURPLE_RPC_PORT', '13495'))
...
def parse_addnode_hosts(conf_path='/bitcoinpurple-config/bitcoinpurple.conf'):
    """Extract addnode hosts from bitcoinpurple.conf"""
    hosts = []
    try:
...
def bitcoinpurple_rpc_call(method, params=None):
    """Make RPC call to BitcoinPurple node"""
    if params is None:
        params = []
...
def get_electrumx_stats(include_addnode_probes=False):
    ...
    try:
        ...
        # Fallback: derive expected network genesis from local BitcoinPurple node
        # if Electrum server.features is temporarily unavailable.
        if not stats.get('genesis_hash_full'):
            try:
                local_genesis = bitcoinpurple_rpc_call('getblockhash', [0])
...
def bitcoinpurple_info():
    """Get BitcoinPurple node blockchain info"""
    try:
        blockchain_info = bitcoinpurple_rpc_call('getblockchaininfo')
        network_info = bitcoinpurple_rpc_call('getnetworkinfo')
        mining_info = bitcoinpurple_rpc_call('getmininginfo')
        peer_info = bitcoinpurple_rpc_call('getpeerinfo')
        mempool_info = bitcoinpurple_rpc_call('getmempoolinfo')
...
def bitcoinpurple_block_height():
    ...
    try:
        height = bitcoinpurple_rpc_call('getblockcount')
        if height is None:
            blockchain_info = bitcoinpurple_rpc_call('getblockchaininfo') or {}
...
def bitcoinpurple_network_hashrate():
    """Get network hashrate (hashes per second)."""
    try:
        hashrate = bitcoinpurple_rpc_call('getnetworkhashps')
        if hashrate is None:
            mining_info = bitcoinpurple_rpc_call('getmininginfo') or {}
...
def bitcoinpurple_difficulty():
    """Get current PoW network difficulty."""
    try:
        difficulty = bitcoinpurple_rpc_call('getdifficulty')
...
def bitcoinpurple_coinbase_subsidy():
    """Get current block subsidy details from node RPC."""
    try:
        subsidy = bitcoinpurple_rpc_call('getblocksubsidy')
        if subsidy is None:
...
def recent_blocks():
    ...
    try:
        ...
        for i in range(min(10, current_height)):
            ...
            block_hash = bitcoinpurple_rpc_call('getblockhash', [height])
            if block_hash:
                block = bitcoinpurple_rpc_call('getblock', [block_hash])
                if block:
                    blocks.append({
                        'height': height,
                        'hash': block_hash,
...
def health():
    ...
    bitcoinpurple_ok = bitcoinpurple_rpc_call('getblockchaininfo') is not None
    stats = get_electrumx_stats_cached(include_addnode_probes=False)
    ...
    return jsonify({
        'status': 'healthy' if (bitcoinpurple_ok and electrumx_ok) else 'degraded',
        'services': {
            'bitcoinpurple': 'up' if bitcoinpurple_ok else 'down',
