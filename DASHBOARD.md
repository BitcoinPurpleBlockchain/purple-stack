...
1. **bitcoinpurple-node** - The BitcoinPurple blockchain node
2. **electrumx-server** - The ElectrumX server
3. **bitcoinpurple-dashboard** - The web monitoring dashboard
...
- **BitcoinPurple Node**: Block height, difficulty, connections, sync progress
...
2. Check dashboard logs:
   ```bash
   docker logs bitcoinpurple-dashboard
   ```
...
1. **BitcoinPurple Node**: May still be syncing. Check logs:
   ```bash
   docker logs bitcoinpurple-node
   ```
...
```bash
docker stop bitcoinpurple-dashboard
```
...
- `GET /api/bitcoinpurple/info` - Node information
- `GET /api/bitcoinpurple/blocks/recent` - Recent blocks
